...
export default function BuildYourStack() {
  ...
  return (
    <div className="w-full relative overflow-hidden bg-mint">
      <AnimatePresence mode="wait">
        {step === 0 ? (
          <motion.section 
            ...
            className="w-full min-h-[80vh] bg-mint flex flex-col items-center justify-center snap-start relative px-6 py-24"
          >
            <div className="flex flex-col items-center justify-center text-center max-w-4xl gap-6">
              ...
              <p className="text-sidebar/80 text-lg md:text-xl max-w-2xl font-medium">
                A fun little estimator to help you imagine what it&apos;ll take to bring your idea to life. One block at a time.
              </p>
              <button 
                onClick={() => setStep(1)}
                suppressHydrationWarning
                className="mt-8 px-8 py-4 bg-sidebar text-white rounded-2xl flex items-center gap-3 hover:bg-black transition-colors font-semibold"
              >
          ...
          <motion.section 
            ...
            exit={{ x: '100%' }}
            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
            className="w-full min-h-[80vh] flex flex-col lg:flex-row snap-start bg-white relative z-10 shadow-2xl"
          >
            {/* Left Column - Form */}
      <div className="w-full lg:w-1/2 min-h-[50vh] lg:min-h-screen p-8 md:p-12 lg:p-16 flex flex-col overflow-y-auto">
        {renderProgressBar()}
      ...
      <div className="w-full lg:w-1/2 min-h-[50vh] lg:min-h-screen bg-mint p-8 md:p-12 lg:p-16 flex flex-col relative overflow-hidden">
